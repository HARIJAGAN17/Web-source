import medzone from '../../assets/medzone.jpg'
import dodgemine from '../../assets/Dodgemine.png'
import fitness from '../../assets/fitnesszone.jpg';
import dietify from "../../assets/dietify.jpg";
import paper from "../../assets/papergram.jpg";
import med3 from "../../assets/med3.0.jpg";

const projects = [
    {
        image:medzone,
        title:"Med-Zone",
        desc:"An accessible personal healthcare web app that uses PoseNet, React, Socket.io, and TensorFlow to let users consult doctors online.",
        link:"https://devpost.com/software/med-zone-q0uwor",
    },
    {
        image:dodgemine,
        title:"Dodge-Mine",
        desc:"A machine learning-powered website application that uses HTML, CSS, JS, and Twilio to detect mine sweepers in the route of sailors.",
        link:"https://devpost.com/software/dodge-mine",
    },
    {
        image:fitness,
        title:"Fitness-Zone",
        desc:"A machine learning-powered web app that uses Google Cloud, Mediapipe, and Python to track your fitness and send you workout reminders via Twilio and Linode APIs.",
        link:"https://devpost.com/software/fitness-zone-9um05f",
    },
    {
        image:paper,
        title:"PaperGram",
        desc:"A web-based blockchain application (Web 3.0) that uses React, web3.js, Metamask, IPFS, and Polygon to safeguard user publications in the research field like IEEE.",
        link:"https://devpost.com/software/fitness-zone-9um05f",
    },
    {
        image:dietify,
        title:"Dietify",
        desc:"A web-based AI-powered diet planning app that uses Flutter and OpenAI API to provide personalized diet plans based on user data.",
        link:"https://devpost.com/software/fitness-zone-9um05f",
    },
    {
        image:med3,
        title:"MedZone 3.0",
        desc:"Accessible healthcare web app with PoseNet, React, TensorFlow for online consultations. Enhanced security and crypto payments with blockchain, Solidity, and Web3.",
        link:"https://health-link.netlify.app/home",
    },
   
]

export default projects;